...
export const TableBody = ({
    ...
    const onRenderItem = (
        ...
        if (statusFieldName === field.EntityPropertyName) {
            ...
            // Custom Display Fields First
            if (field.InternalName === "ApplicantName") {
                return <p>{item.ApplicantName ?? "-"}</p>;
            }

            if (field.InternalName === "ApplicantDepartment") {
                return <p>{item.ApplicantDepartment ?? "-"}</p>;
            }

            if (field.InternalName === "GuestStatus") {
                return <p>{item.GuestStatus ?? "-"}</p>;
            }

            if (field.InternalName === "RequestDate") {
                return (
                    <p>
                        {item.RequestDate
                            ? new Date(item.RequestDate as string).toLocaleDateString("en-GB")
                            : ""}
                    </p>
                );
            }
            ...
            {data.map((item) => (
                <tr key={item.Id as number}>
                    ...
                    <td>
                        <a
                            className="item-link"
                            href={`${detailsFormUrl}?formId=${item?.ParentID}`}
                            target="_blank"
